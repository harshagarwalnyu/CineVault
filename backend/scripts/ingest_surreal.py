...
class TropeExtraction(BaseModel):
    name: str = Field(
        description="The name of the trope, pacing, or mood, e.g., 'Enemies to Lovers', 'Fast-Paced', 'Cyberpunk Noir'"
    )
    description: str = Field(
        description="A brief explanation of what this trope means generally."
    )
    confidence: float = Field(
        description="How confident you are this trope applies to this movie (0.0 to 1.0)."
    )
    explanation: str = Field(
        description="Exactly how and why this trope appears in THIS specific movie."
    )
...
async def main():
    ...
    for index, row in sample_df.iterrows():
        ...
        popularity = (
            float(row["popularity_score"]) if "popularity_score" in row else 0.0
        )
        ...
        try:
            ...
            print(
                f"✓ Successfully ingested {title} and linked {len(tropes_data.get('tropes', []))} tropes."
            )
