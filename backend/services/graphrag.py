...
class GraphRAGService:
    ...
    async def agentic_discovery(self, query: str):
        ...
        matched_trope_ids = await self._vector_search_tropes(
            intent.target_tropes + [intent.mood]
        )
        ...
        for r in results[0]["result"]:
            ...
            if movie_data and movie_data[0]["result"]:
                ...
                reasons = (
                    edges_data[0]["result"]
                    if edges_data and edges_data[0]["result"]
                    else []
                )
        ...
        accessible_recommendations = await availability_agent.filter_accessible_only(
            recommendations
        )
