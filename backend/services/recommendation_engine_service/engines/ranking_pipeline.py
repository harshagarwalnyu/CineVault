    ...
    def rank(
        ...
        try:
            ...
            if tt_engine.is_ready and user_id is not None:
                ...
                for rank, mid in enumerate(tt_candidates):
                    ...
                    source_scores.setdefault(mid, {})["two_tower"] = 1.0 - rank / max(
                        len(tt_candidates), 1
                    )
        ...
        if session_movie_ids:
            try:
                ...
                if sess_engine.is_ready:
                    sess_candidates = sess_engine.get_candidates(
                        session_movie_ids, k=50
                    )
                    for rank, mid in enumerate(sess_candidates):
                        candidate_ids.add(mid)
                        source_scores.setdefault(mid, {})["session"] = 1.0 - rank / max(
                            len(sess_candidates), 1
                        )
            except Exception as e:
            ...
            try:
                ...
                if hstu.is_ready:
                    ...
                    for rank, mid in enumerate(hstu_candidates):
                        candidate_ids.add(mid)
                        source_scores.setdefault(mid, {})["hstu"] = 1.0 - rank / max(
                            len(hstu_candidates), 1
                        )
            except Exception as e:
        ...
        try:
            ...
            if gcn_engine.is_ready and user_id is not None:
                ...
                for rank, mid in enumerate(gcn_candidates):
                    candidate_ids.add(mid)
                    source_scores.setdefault(mid, {})["lightgcn"] = 1.0 - rank / max(
                        len(gcn_candidates), 1
                    )
        except Exception as e:
        ...
        try:
            ...
            if temporal.is_ready and user_id is not None:
                ...
                for rank, mid in enumerate(temp_candidates):
                    candidate_ids.add(mid)
                    source_scores.setdefault(mid, {})["temporal"] = 1.0 - rank / max(
        ...
        if mood_text:
            try:
                ...
                mood_recs = mood_engine.get_mood_recommendations(
                    mood_text, rec_engine.movies_df, limit=100
                )
        ...
        if session_movie_ids:
            try:
                ...
                if clrec.is_ready:
                    clrec_scores = clrec.score_candidates(
                        session_movie_ids, candidate_list
                    )
        ...
        try:
            ...
            if bandit.is_ready:
                sampled = bandit.select_arms(
                    candidate_list, user_id=user_id, k=len(candidate_list)
                )
        ...
        for mid in candidate_list:
            ...
            collab = (
                rec_engine.get_collaborative_score(user_id, mid) / 10.0
                if user_id
                else 0.5
            )
        ...
        try:
            from backend.services.recommendation_engine_service.engines.mmr_engine import (
                get_mmr_reranker,
            )
        ...
        try:
            from backend.services.recommendation_engine_service.engines.serendipity import (
            ...
            if user_id and rec_engine.ratings_df is not None:
                user_rated = set(
                    int(r)
                    for r in rec_engine.ratings_df[
                        rec_engine.ratings_df["user_id"] == user_id
                    ]["movie_id"]
                )
                ...
                if _pd.notna(release_dt):
                    ...
                    if age_years < 3:
                        ...
                        movie["hybrid_score"] = round(
                            movie.get("hybrid_score", 0) * boost, 1
                        )
        ...
        try:
            from backend.services.recommendation_engine_service.engines.explainability import (
            ...
            for movie in ranked_list:
                if not movie.get("reason"):
                    movie["reason"] = explain_engine.explain(
                        movie,
                        source_scores.get(movie["id"], {}),
                        {
                            "query": query,
                            "mood": mood_text,
                            "user_id": user_id,
